...
local M16 = Class:derive("M16") -- needs to change for each gun
...
function M16.create_sprite(atlas)
    -- we make this like sprite.height/2 on the sprite height
    

    
    local still_anim = Anim(0, 0, 38, 16, 1, 1, 8, false)
    if atlas == nil then
        assert(false, "no atlas supplied to sprite!")
    end

    local spr = Sprite(atlas, 36, 16, nil, true)
...
function M16:still_enter(dt)
    self.sprite:animate("still")
end

function M16:still(dt)

end

function M16:still_exit(dt)
...
-- function M16:updatePost(dt) end
